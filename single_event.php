...
require('connect.php');
$event_id=$_GET["e"];
$sql='SELECT event_id,date,time,venue.name as v_name,city,country,sport.name as s_name,competition.name as c_name, status,description,capacity
...
$query=$connection->prepare("SELECT name,city,country FROM teams_playing join team on teams_playing._team_id=team.team_id LEFT JOIN location on location.location_id=team._location_id WHERE _event_id=:event");
$query->execute(["event"=>$event_id]);
